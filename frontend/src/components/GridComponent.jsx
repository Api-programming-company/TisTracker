...
const GridComponent = ({values}) => {
    ...
    return (
        <div className="custom-grid" style={{gridTemplateColumns: `repeat(${n_columns},1fr)`}}>
            {Object.keys(values[0]).map((header,index) => <div className='custom-grid-header' style={{backgroundColor: localStorage.getItem('theme') === 'dark' ? '#f00' : '#dadada'}} key={index}>{header}</div>)}
